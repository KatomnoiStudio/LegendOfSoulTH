...
Mirrors `ring0-traffic-control-law.md`'s merge break-glass — same eligibility, same logging discipline, extended to claiming specifically:

- If Ring 0 is genuinely unavailable **and** the requesting dev already holds `admin:true` on the GitHub repo (today: `nustanakritwithai`, `DemoGODRTX` — verify current admins via `gh api repos/.../collaborators`, not from memory), that dev may self-claim the old way instead of waiting on a lock: `git fetch origin` → edit their picked row directly (`Owner`/`Status: 🟡 claimed`/`Claimed`) → commit **that edit alone** (`docs(tasks): claim #NN <task name> [break-glass]`) → push.
