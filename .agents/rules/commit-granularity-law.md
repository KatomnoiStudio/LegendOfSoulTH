...
> **Scope**: Binding for every agent, every machine — Ring 0 and Ring 1 alike.
...
- "Merge the battle system from Hih#11" → one commit for the merge content itself; separate commits for the *conflict-resolution merge commits* that followed are fine (those are git's own mechanism, not a violation — see "What this doesn't mean" below).
...
- **Don't hold work uncommitted** to wait for a "bigger" batch either — per `pre-push-sync-law.md`, verify and push each completed task promptly. This rule is about *shape* (one task, one commit), not about delaying commits to accumulate several tasks into one.
