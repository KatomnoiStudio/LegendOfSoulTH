...
- A crash screen tells the player the truth about where their data is, and **offers no control it cannot honour**. `ErrorBoundary` used to carry a "back up to file" button; it failed 100% of the time in two successive incarnations (a dormant repository, then a hardcoded `ok:false` stub in the live one), so it was removed rather than relabelled — a frightened player pressing a button that always errors is worse off than one who was never offered it. The screen states that progress is server-side and offers only reload. A backup button may return when `accountRepository.supabase.ts` can actually produce a file; a test pins its absence until then. Note also that anything `ErrorBoundary` imports must not reach `supabaseClient` at module scope — `main.tsx` imports the boundary before `createRoot`, and `supabaseClient` throws on missing env, which is the exact failure its dynamic `import('./App.tsx')` exists to prevent.
- **`reportError` must never be able to throw.** It is the last-resort net, and `globalErrorHandlers` feeds it values no one controls (`event.error ?? event.message`, `event.reason`). Reading `err.message` is not a safe read — it invokes a getter, and a revoked Proxy, a cross-origin `Window`, or a hostile object throws from it. Every property read goes through `readProperty` in `normalizeError.ts`, and `reportError` wraps the whole normalize step. If the reporter throws, the report is lost, the `visible` relay never runs, and the player gets a dead screen with no code to quote — strictly worse than no reporter at all.
