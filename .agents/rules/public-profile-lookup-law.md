...
2026-08-08: `findPlayerByUid` (then at `src/data/accountRepository.supabase.ts:322`, now `:413`) queried the `profiles` table directly, filtered by public UID. The table's only SELECT RLS policy is `auth.uid() = id` (own row only), so the lookup returned 0 rows for anyone else in production — the "add friend by UID" feature had never worked. Two fixes were proposed (a public-read RLS policy/view on `profiles`, or a `SECURITY DEFINER` RPC matching the existing currency-RPC pattern); a 4-seat blind opinion review (seats: realtime, reality, feeling, outdim) refuted both as unsafe if implemented naively:

**Status: RESOLVED.** `supabase/migrations/0012_public_profile_lookup.sql` implements the RPC shape rules 1-4 below require (`find_player_by_uid`, explicit `returns table(uid, name, level, title)`, `revoke execute ... from public, anon` / `grant execute ... to authenticated`, rate-limited via the shared 0011 helper), and `findPlayerByUid` in `accountRepository.supabase.ts` now calls it instead of querying `profiles` directly. The incident record below stays as the reasoning trail for why the rules exist; it is history, not a live bug.
