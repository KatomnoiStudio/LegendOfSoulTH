<!-- coalmine: verified 2026-08-08 · exemplar this project's own PR #19/#21/#22/#23/#24 merge history + the real PR #19 silent-regression incident (item 90) + GitHub's native merge queue (atomic remote merge, no local-only merge state) + Bors/Mergify (queue pause/priority) · gaps found via a CoalBoard 4-seat opinion-lane sweep (2026-08-08), each spot-verified against live repo state before being filled here · revalidate 90d -->
...
**Path B's status surface (found via a CoalBoard opinion-lane sweep, 2026-08-08):** Path A gets a free status channel — the PR page. Path B doesn't, by construction (no branch name, no PR number to watch). **`TASKS.md`'s own row for the dev's claimed task IS the canonical Path-B status surface** — Ring 0 keeps that row's `Status`/notes current enough that a Path-B dev can answer "did my thing land?" by reading their own row, not by asking again. This doesn't require a new channel, only that the existing ledger gets treated as one.
...
- **Single point of failure**: this design does not remove Ring 0 as a bottleneck — as more devs join, a serialized controller caps total throughput. Not solved here; revisit if it becomes the real constraint, not before (no scaling ceremony for a problem that doesn't exist yet). **Reconsidered, not just left stale (CoalBoard opinion-lane sweep, 2026-08-08):** a formal backup-merger role was weighed against real-world exemplars (GitHub merge queue's batching gets throughput via CI automation, not a second human authority) and explicitly declined for now — a rarely-exercised backup atrophies exactly when a real outage forces its use (the board's own blind `outdim` seat flagged this independently), and no second trusted `admin:true` account currently exists to hold the role anyway (see the Controller offline section above). Still correct to revisit only if throughput becomes the real constraint — this entry records that the question was asked, not ignored.
- **Who reviews the controller's own merges**: this law doesn't add a check above Ring 0's own merge judgment. Existing practice (post-hoc ask-CB review) is the closest thing that exists; not a full answer. GitHub-native review-authorization (Bors' `r+`, GitHub's own required-reviewers) doesn't close this gap here either — HetCreep is both the sole `admin:true` account and the author of self-merged commits, so even adopting that mechanism wouldn't produce independent review of Ring-0-authored work without a second admin.
- **No technical backstop for this law today**: `ring0-authority.md` says outright that nothing stops a non-compliant push, and `enforce_admins` on this repo's branch protection is currently `false` — all admin accounts can bypass this entire procedure by pushing straight to `master`. This law binds by convention only. Flagged here rather than implied to be enforced; whether to flip `enforce_admins` is a real product/access decision for HetCreep, not something this rules file can decide for itself.
- **CI doesn't cover the exact asset class PR #19 regressed** (harden-runner / SHA-pinned actions / SBOM presence in `.github/workflows/*.yml`): `npm run ci` doesn't parse workflow YAML, so the merge procedure's fresh-tip re-check (step 2 above) is currently 100% human-judgment for this specific class, the same failure mode the re-check step exists to close for everything else. A lightweight CI assertion (grep for pinned SHAs / harden-runner presence) would be proportionate — YAGNI-compliant, unlike full merge-queue tooling — but building it is out of scope for this rules file; named here as a follow-up, not implemented.
