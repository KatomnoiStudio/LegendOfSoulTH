...
**The ADDRESSING rule (owner ruling 2026-08-09, "สิ่งนี้สั่งใครกันแน่"):** main and every hired seat run at the SAME cwd, so the auto-load (the up-tree governance walk + hooks) flows into everyone — that cannot be prevented, and the belt does not pretend to prevent it. What IS defined is the ROUTE: **a law binds a seat only if it is ADDRESSED to that seat.** Per-seat binding order: (1) the dispatch prompt (the task contract) → (2) the seat's own file (`.claude/agents/belt-*.md`) + its registry row → (3) the law LAYER addressed to that seat → (4) everything else auto-loaded = background context, data not command. Concretely: **Layer 1's authorities (routing, merge, bump, dispatch) address the BELT-END SEAT — main alone**; a caretaker reading the same file gains none of them. **Worker-seat duties** (one topic + per-item verdicts, contract walls, memory duties, the gate law, the language rule, never editing Ring-0 rule files — the prohibition rides the ROLE, not the ring) address EVERY worker seat — Ring-0-hired and Ring-1 alike. **Layer 2's intake rules** address Ring-1 devs specifically. **The addressing rule allocates the BELT's layered authorities ONLY — the repo's baseline laws (`AGENTS.md` rules 1–22 + `.agents/rules/**`) bind every seat regardless of addressing (owner ratification 2026-08-09, closing the gate's F1: without this clause, worker seats could read rule 22's RLS law or rule 15's CUT/DEFERRED lock as tier-4 context).** Layer 1's PROHIBITIONS that name every agent (e.g. no agent applies to production) and its dispatcher duties (the pin + never-delegate-sensitive-down) bind WHOEVER holds that role — including a caretaker acting as Joiner.
...
- **Quota death is TRANSIENT, never a fallback trigger (owner correction 2026-08-10, "ฉันสั่งคุณบินต่อ คุณก็ไปสั่ง claude -p บินต่อสิ"):** a worker killed by the account limit is NOT a missing caretaker — its sid and transcript survive, and the spend already made bought CONTEXT, not nothing. The move is wait-for-reset → re-dispatch the SAME sid. Main's deepest-knowledge fallback never fires for a temporarily-unavailable caretaker; main making a live caretaker's work is the pack-mule failure by another road. (Main did exactly this once, same day — owner caught it; the caretakers were re-dispatched to reclaim and own the work.)
...
- **Main's working method (owner ruling 2026-08-09): the task queue, always.** Every incoming task lands in the task system the moment it arrives — nothing is held in main's head. A NEW task re-triages the WHOLE queue into three urgency bands: **เร่งด่วนมาก (P1) · เร่งกลาง (P2) · เร่งน้อย (P3)**. Main queues, bands, and proposes — then **WAITS FOR THE USER'S SWITCH**: no queued item starts executing until the owner flips it (names the item or the band). The queue is the proposal; the switch is the consent. `TASKS.md` stays the durable repo ledger on top (rules 16/20 unchanged). Two riders (same ruling, extended same day):
  ...
  - **Continuous queue cleanup, with a HARD LIMIT (owner ruling 2026-08-10, set after 3 completed tasks were caught lingering):** a completed task is SWEPT (deleted) at the very next queue touch — its record already lives in `MEMORY.md`/git, the queue holds nothing. **Ceiling: at most 2 completed tasks may exist at any moment; reaching 3 = the violation itself**, sweep immediately and note the miss. Superseded/stale deleted, duplicates merged, every visit; a rotting queue misroutes the switch. **Cleanup is MAIN's OWN duty** (owner ruling, same day: "ให้ main ทำหน้าที่ cleanup") — never delegated, never assumed done by a worker.
  - **DONE requires SHIP-verification (owner ruling 2026-08-10: "จะ mark ว่า done ได้ต้องตรวจสอบสิ่งที่ ship"):** a task is marked completed only after the SHIPPED artifact is verified — the gate it cleared named, and the landing itself checked (the commit on the remote, the deploy state, the migration verified read-only, the behavior observed — whichever the task's output is). "Code written" is not done; "pushed" is not a report; verification of the ship IS the completion bar. **Worker DONE verdicts (owner ruling 2026-08-10, answering the gate's scope question): the verification seat is MAIN, on the COMMIT STREAM — everything on the belt already flows to main, and main verifies a worker's DONE against what actually landed there. On an incomplete return, or any DONE main doubts, main FIRES a completion-challenge at the caretaker ("เรื่องนี้ทำเสร็จ ทำครบแล้วหรือยัง") and the caretaker answers with evidence — the challenge is main's standing right, no new worker self-duty is created.**
- **cwd hygiene (owner ruling 2026-08-10, task #22): anything that has outlived its purpose is cleaned up** — INSPECT contents before removing, always; empty folders left behind by a cleanup are verified (hidden files included) then removed in the same pass. Turn limits (main-set per owner delegation): the obsolete-check rides every queue re-band; an identified-obsolete item is swept within 5 main-turns or justified; a full-tree sweep runs at least once per session AND before every release-train departure. Binds every worker seat on every ring — Ring-1 clones carry the same duty.
- **`MEMORY/` lifecycle (owner ruling 2026-08-09, "โดนปลุกให้ load / return ให้ตกผลึก + บีบอัด + อัพเดตข้อมูลทั้งหมด"):** a seat WOKEN (spawned/resumed) LOADS its own `MEMORY/NN` file first; on RETURN it CRYSTALLIZES (lessons, decisions, live state — never transcript), COMPRESSES (rewrites in place, dedupes, tightens), and UPDATES everything stale. **Cut aggressively — the file may shrink to near-nothing: git log holds all history.** The file carries only what the seat needs at its next wake; memory is working knowledge, never an archive. Precondition of the safety net: a return's memory file is COMMITTED before the next wake compresses it — uncommitted content has no git copy behind it.
...
- **Ring-1 layer (intake rules command outside devs; its worker-shaped duties bind every worker seat per the addressing rule above):** PR-only intake (§1b — code arrives as a PR or it has not arrived), one topic per claim (rule 16), contract walls (`does NOT own` lines are fences, not documentation — §6), memory duties (root `MEMORY.md` push-with-submit; `MEMORY/` per-caretaker), never editing Ring-0 rule files. **Amended (owner ruling 2026-08-09): submitting the PR IS the whole duty — no review-request step, no waiting for approval, no author-side fix round (the adopt-inward rule replaced that flag-and-fix loop entirely). The one prohibition AFTER SUBMISSION: MERGING — even where GitHub permissions would technically allow it (admin collaborators). Merge authority is the belt end's alone; rule 18's logged break-glass is the only exception.**
