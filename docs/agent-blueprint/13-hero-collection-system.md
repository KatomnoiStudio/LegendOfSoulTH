...
3. Every `ROSTER` entry has a `role` distinct from every other entry — currently true for all 7, including Erlang Shen's lightning-spear role; a data-lint/test asserts this (`src/data/heroCollection.test.ts`), so §4.1's "no reskin" rule is machine-checked, not just eyeballed. Production Batch 01 keeps its stricter archetype-uniqueness check separately, while Erlang Shen remains outside that batch.
