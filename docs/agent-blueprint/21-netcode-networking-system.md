...
- **Out (session end):** a PvP match result — **not `RealtimeBattleResult` unmodified**. That type (`types.ts:179-191`) is entirely PvE-shaped — `stageId`, `stageName`, `defeatedEnemyIds`, `earnedExp`, `earnedGold`, `droppedItems`, sourced via `BattleResultAdapter.ts` from `RewardSystem`'s per-enemy/per-wave reward calc — with no field for `matchId`, opponent identity, or a two-sided win/loss. P12 defines `RealtimePvPResult` with both player IDs, outcome reason, final authority hash, and authority time. P13 may consume that server result; it must never trust a client-reported win.

### Dependencies

- **Combat/realtime-battle system** — hard dependency; reuses its simulation wholesale, does not reimplement it (§6.3). The shared `RealtimeBattleEntity` type has since been extended for PvP — `EntityType` now includes `'pvp-player'` (`realtimeBattle/types.ts:47`), and a `controllerId?: string` field documented as "Auth profile that controls a PvP combatant; absent for PvE entities" (`types.ts:105`) represents an opposing player's hero. `PvPAuthorityEngine.ts`'s `prepareCombatant` sets `entityType: 'pvp-player'` and `controllerId: playerId` for both participants (`PvPAuthorityEngine.ts:138-139`), and §3.8.7's hit-reaction tier is resolved by hanging `combatTier: 'elite'` off that same combatant (`PvPAuthorityEngine.ts:145`), asserted by a passing test (`PvPAuthorityEngine.test.ts:93-101`).
