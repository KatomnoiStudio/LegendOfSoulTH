...
- **In:** `BattleWaveDefinition.enemies[]` (`stageConfig.ts:160-163`) marks one spawn's `templateId` as the mini-boss for that wave via `MiniBossStageParams.templateId` (`stageConfig.ts:192-195`) — no schema change, just data (e.g. `trial-08`, `stageConfig.ts:754-755`).
- **Out:** `EnemyBrain`/`RealtimeBattleEntity` carry the resolved tier (`RealtimeBattleEntity.tier`/`combatTier`, `types.ts:101,113`) so `EnemyAISystem.stepEnemyAI` and the damage/hitbox knockdown gate (`isKnockdownEligible` in `DamageSystem.ts:36`) branch on it — `EntityState` already includes `'knockdown'`/`'getUp'` (`types.ts:29-30`).
...
3. Regression test: an Elite entity hit by a knockdown-flagged move enters Knockdown + 200ms i-frame (§3.6.12 line 305) identically to a `'boss'` entity; a `'normal'` entity under the identical hit does not knock down (pins the tier gate per AGENTS.md rule 12).
...
Add `tier` as **one field on the existing `RealtimeEnemyTemplate` table** (`stageConfig.ts:35-70`), not a new `EliteEnemy` class/interface or a parallel AI system — mirrors this codebase's own stated rationale in `EnemyAISystem.ts:16-18` for refusing to fork the movement system ("จะกลายเป็นระบบเดินชุดที่สองที่กฎไม่ตรงกัน") and the file's explicit single-source-of-truth comment (`stageConfig.ts:14`, "ห้าม hard-code ข้อมูลด่านไว้ใน Component"). One flag, consumed by one shared knockdown gate — no premature Elite/Boss class hierarchy for what is currently a stat-multiplier + boolean.
