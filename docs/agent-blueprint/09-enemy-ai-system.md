...
`EnemyAISystem.ts` (`src/game/realtimeBattle/EnemyAISystem.ts`) owns per-enemy **decision-making** only: given one enemy, the player, and a time delta, decide the next `EnemyAIState` (`idle → chase → attack → recover`, plus `hit`/`dead` interrupts) and which direction to move. It does not own movement execution (`MovementSystem.ts` applies the returned vector, collision, arena clamp), damage application (`DamageSystem.ts` / `RealtimeBattleRuntime.resolveEnemyAttack`), hitbox/hit detection (`HitboxSystem.ts`), or rendering/telegraph VFX (none exists yet — see Stay-current note). Per-enemy tuning data (ranges, cooldowns, HP/atk/def) lives in `stageConfig.ts`'s `RealtimeEnemyTemplate`; attack timing comes from the single shared `ENEMY_ATTACK` in `attacks.ts` (`src/game/realtimeBattle/attacks.ts:196-210`) — every enemy currently uses the same attack, there is no per-enemy-type or per-tier attack set yet.
...
- **combat** (Hit Reaction System) — reads `enemy.hitStunRemainingMs` to enter `hit` state (`EnemyAISystem.ts:92-96`); no knockdown/getUp support exists here yet.
...
- A literal `Telegraph` phase distinguishable from `AttackActive` (§3.6.8's `Idle→Chase→Telegraph→AttackActive→Recovery`) — today `attack` is one combined brain-state covering `startupMs+activeMs+recoveryMs` as one timer (`ATTACK_TOTAL_MS`, `EnemyAISystem.ts:31,104-109`). Splitting this is a real gap for renderer/UI consumption, but it's a brain-state change this contract can make; it does not require the renderer itself.
...
- Knockdown/GetUp states (§3.6.8, elite/boss + flagged moves only, 200ms i-frame per §3.6.12) — `EntityState` (`types.ts:25`) has no `knockdown`/`getUp` value today, and neither `RealtimeEnemyTemplate` nor `AttackDefinition` carries data that would ever drive a brain into one. This depends on the Elite/Mini-boss Tier System (#10) that doesn't exist yet (see Dependencies) — adding unreachable states ahead of it is scaffolding, not a testable unit of this contract.
...
Genuine, blueprint-grounded churn: `EnemyAISystem.ts` is currently a single flat state machine shared by every enemy tier (no tier field, no phase concept). §3.8.3/§3.8.4 (per `AGENT_BLUEPRINT.md:55`) already source Elite/Mini-boss Tier System (#10) as reusing this core — so when systems #10/#11 are implemented, this file will need to grow tier-aware branches (Knockdown/GetUp, PhaseTransition/Invulnerable) without duplicating the state machine. Not speculative — it's an already-cited future requirement on this exact file, tracked but not yet actionable (see Done-criteria's forward-looking gaps).

### Low-maintenance-cost design

Keep extending the **existing single-source data files** instead of branching the state machine per enemy type: add `telegraphMs`/`knockdown` fields to `AttackDefinition` (`attacks.ts`, which already carries the file's own "ห้ามกระจายไปเขียนใน component" single-source-of-truth comment, lines 1-5) and a `tier`/attack-set field to `RealtimeEnemyTemplate` (`stageConfig.ts`), rather than writing a second `EnemyAISystem` for elites/bosses. This directly matches the project's own already-stated reasoning in `EnemyAISystem.ts:9-11` (reusing `MovementSystem` instead of a second movement implementation) and §3.8.3/§3.8.4's core-reuse framing for #10 — one state machine, data-driven per enemy, no premature per-tier abstraction until #10/#11 actually need it.
