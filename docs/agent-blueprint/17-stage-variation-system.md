...
Owns: the definition of a stage's **type** (Survival, Defend, Chase, Hazard, Mini-boss, Time Attack, Custom per §5.2) and that type's win/lose condition + pacing target (normal 2–5 min, boss 5–8 min, §5.1). Does **not** own: enemy AI behavior (`EnemyAISystem.ts` resolves encounters via its `EnemyAIState` machine — `'idle' | 'chase' | 'attack' | 'recover' | 'hit' | 'dead'`; §3.6.8 locks a design-intent Idle→Chase→Telegraph→AttackActive→Recovery sequence, but the code today collapses Telegraph/AttackActive into a single `'attack'` state timed by `ENEMY_ATTACK_TIMING`, and `DamageSystem.ts` doesn't yet branch on it — that split is not wired up), reward math (`RewardSystem.ts` owns EXP/gold/drop calculation, §5.3), inter-stage gating/stamina cost (§5.1 explicitly OPEN, needs a HetCreep call, not this system's decision), or numeric difficulty tuning (data-table values, deferred to P7/P11 per §5.1).

**Current code state: this system does not exist yet.** `src/game/realtimeBattle/stageConfig.ts:56-76,140-186` defines only the plain `Wave → Wave` pattern — the exact "repetitive arena waves" §5.2 says NOT every stage should be. `REALTIME_STAGES` has two stages (`trial-01` one wave, `trial-02` two waves); no `stageType` field, no Survival/Defend/Chase/Hazard/Mini-boss/Time-Attack/Custom win-condition logic anywhere in `src/game/`. The exploration/stage-select layer that would host a `BattleContext.stageId` picker is fully commented out (`src/game/flow/types.ts:1-18`, disabled 2026-08-07 per HetCreep) — currently both lobby buttons open `LobbyBattleSession` directly with no stage-type concept in the path at all.
