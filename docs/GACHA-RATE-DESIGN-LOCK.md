...
> **⚠️ คอลัมน์ขวาสุดคือ _ค่าที่ ship อยู่ตอนเขียนกฎนี้_ ไม่ใช่ค่าที่ล็อก — ทุกช่องที่เขียนว่า OPEN
> ปิดไปแล้วทั้งหมดเมื่อ 2026-08-12/13 · §12 คือสถานะของจริง ค่าที่ล็อกอยู่ใน §11.x**
>
> เจตนาของคอลัมน์นั้นคือบันทึกช่องว่างระหว่างกฎกับของที่ ship — ไม่ใช่ประกาศค่า และค่าไม่ได้ถูก
> เขียนซ้ำที่นี่โดยตั้งใจ (กฎห้ามเลขซ้ำ) **แต่การเขียนว่า OPEN ค้างไว้ทำให้มันโกหก** จึงแก้ป้าย
> ให้ชี้ไปที่ section ที่ปิดมันแทน (2026-08-15)
...
| `B[type][r]`         | band rate ต่อ banner type                               | `SUM = 1` พอดีที่ rho, `0 < B[top] < 1`, ไม่อ่าน pool size / จำนวนตัวละคร / asset readiness                  | `legendary 0.05 / epic 0.25 / rare 0.70` — **ปิดแล้ว §11.1** — ค่าที่ล็อกอยู่ที่นั่น                                                         |
...
| `d`                  | discount ของ multi                                      | หนึ่ง scalar ต่อ banner, default 0                                                                           | **ไม่ได้ประกาศ** — `900/1000` แปลว่ามี `d = 0.10` ซ่อนอยู่ **ปิดแล้ว §11.6** — `d = 0`, `cost_multi` ตกจาก `c` เสมอ                          |
| `c`                  | cost_single                                             | หนึ่งค่าต่อ banner ในสกุลที่ banner ประกาศ                                                                   | `100 gem` — **ปิดแล้ว §11.8** — ค่าที่ล็อกอยู่ที่นั่น                                                                                        |
| `P[r]`               | pity threshold ต่อแบนด์ หน่วยเป็น pull เต็ม             | `infinity` = แบนด์นั้นไม่มี counter; ต้องผ่าน I9                                                             | `P[legendary] = 30` — **ปิดแล้ว §11.5** — ค่าที่ล็อกอยู่ที่นั่น                                                                              |
| `tau_min`, `tau_max` | ช่วง reach fraction ที่ยอมรับได้                        | `tau_min <= (1-q)^(P-1) <= tau_max`                                                                          | **ปิดแล้ว §11.5** ค่าที่วัดได้วันนี้คือ 22.594%                                                                                              |
| `S[r]`               | shards ต่อ duplicate ของ rarity r                       | หนึ่งแถวต่อ rarity, source-agnostic (wish/shop/grant/compensation ใช้อันเดียวกัน), นิพจน์เดียว               | `1` ทุก rarity — **ปิดแล้ว §11.10/§11.10b** — ค่าที่ล็อกอยู่ที่นั่น                                                                          |
| `L[r][k]`            | shards ที่ต้องใช้ถึงดาว k                               | หนึ่ง array หนึ่งบ้าน; รูป rarity-blind (`L[r][k] = L[k]`) ยังถูกกฎ **ก็ต่อเมื่อ** ผ่าน I13                  | `1/2/4/8/12` (สะสม 27) — **ปิดแล้ว §11.11** — ค่าที่ล็อกอยู่ที่นั่น                                                                          |
...
ทุกตัวเลขข้างล่างคือ **output ของสูตร §2** ไม่มีค่าไหนวางด้วยมือ input คือ `B = 0.05/0.25/0.70` (**ค่าตอนเขียน — ปิดแล้วที่ §11.1**), `u = 0`, `w = 1` ทุกตัว, `P[legendary] = 30`, `c = 100`, `K = 10`, `d` ยังไม่ประกาศ, `S = 1`, `L` สะสม 27, `rho = 7`
...
```
I1 ผ่าน · I2 ผ่าน · I3 ผ่าน · I4 ผ่าน · I5/I5b/I5c ผ่าน · I7 ผ่าน · I9 ต้องรอ tau (**ปิดแล้ว §11.5** — ตัวเลขในหมวดนี้เป็นภาพตอนเขียน)
I12 ผ่าน · I13 ผ่าน
...
| `BATCH_01_GACHA_BANNER` (costGems 160, softPityAt 74, hardPityAt 90, weights 5/12/12/18/18) | src/game/heroes/gachaPool.ts:17-25 — **ไฟล์ถูกลบแล้ว** (ดู §11.15) แถวนี้เป็นบันทึกว่าเคยมีอะไร ไม่ใช่ตำแหน่งที่ยังอ่านได้                           | ต้องเป็น banner row จริง (มี `B`, `c`, `P`, `w` ของตัวเอง) หรือถูกลบ — วันนี้ไม่มีใครอ่านตัวเลข (§8.10)                              |
...
เอกสารนี้ **บรรทัดถัดไปเป็นภาพตอนเขียน — §8 ปิดครบ 12/12 แล้ว ดู §12**
ตอนเขียนยังเหลือข้อ 2-14 เปิดอยู่ และข้อ 2 มีใบสั่งงานแยกที่ `ASSIGN-PITY-P-TAU.md`
ซึ่งต้องอัปเดตให้ตรงกับ `B[top]` ที่ล็อกที่นี่ก่อนส่งจริง **(ไฟล์นั้นไม่มีอยู่ใน repo แล้ว —
ข้อ 2 ปิดที่ §11.5 แทน ใบสั่งงานจึงหมดหน้าที่ไปเอง)**
...
**ลิสต์ "ยังเปิด" ข้างล่างเป็นภาพของวันที่ 2026-08-12 — ปิดครบหมดแล้ว ดู §12** ตอนนั้นยังเปิด: 6 (`L[r][k]`), 7 (epic pity), 8 (soft pity vs blueprint), 9 (`u`),
