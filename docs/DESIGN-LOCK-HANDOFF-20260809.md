...
**ตอบไปแล้ว 10 จาก 12 — เหลือ 2 ข้อที่ยังค้างจริง**
...
| **4**                | ⏳ **ยังค้าง — และแย่ลง**                          | `stepAllyAI` มี production caller **2 ตัว** แล้ว (`RealtimeBattleRuntime.ts:440`, `PvPAuthorityEngine.ts:438`) ตอนเขียนข้อนี้มีตัวเดียว · contract 07 ยังเขียนว่า summon "hand off ... verbatim, no bespoke AI"                                                                                                     |
| **12**               | ⏳ **ยังค้าง**                                     | contract 14 กับ 18 ยังอ้างสิทธิ์ `applyBattleExp` ที่ `RewardSystem.ts:127` ทับกันทั้งคู่                                                                                                                                                                                                                           |

ทั้ง 4 และ 12 เป็น **design fork จริง** ไม่ใช่การแก้เอกสารให้ตรงโค้ด — ต้องรอ HetCreep เลือก ไม่ใช่งานที่ main ตัดสินแทนได้
