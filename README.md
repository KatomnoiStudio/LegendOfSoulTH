...
- The character, UI, and background art was **generated by AI**, then trimmed and fitted to the code by the owner by hand in an image editor (mechanical only — no drawing, retouching, or generative editing) and cut into animation frames and cleaned up in-repo by the scripts under `tools/`. Some generations used reference or source images, and their origins were not recorded, so the project **cannot currently claim the art is free of third-party input.**
- The 8 sound effects in `public/audio/sfx/` are third-party: Kenney.nl, CC0 (public domain, no attribution required).
- Characters drawn from myth/history (Hanuman, Lu Bu, etc.) are public-domain figures; no copyrighted character _design_ appears directly, and where that constraint matters it's reinterpreted as an original character instead (e.g. "Astra Vale — Cosmic Force Warrior").
