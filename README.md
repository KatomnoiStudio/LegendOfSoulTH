...
- The character, UI, and background art was **produced with AI tools**, then trimmed and fitted to the code — mechanical work only, no drawing or retouching, and nothing done by hand — and cut into animation frames and cleaned up in-repo by the scripts under `tools/`. Some generations used reference or source images, and their origins were not recorded, so the project **cannot currently claim the art is free of third-party input.**
- **Blender was also used**, so an unknown share of the art may be **renders of 3D models** rather than generated images. Whether those models were built from scratch or downloaded — and if downloaded, under what licence — **is not recorded**, and neither is which files came through Blender.
- The 8 sound effects in `public/audio/sfx/` are third-party: Kenney.nl, CC0 (public domain, no attribution required).
- Characters drawn from myth/history (Hanuman, Lu Bu, etc.) are public-domain figures. Where a modern copyrighted design was the obvious reference point, the project's stated intent is to reinterpret rather than reproduce it (e.g. "Astra Vale — Cosmic Force Warrior"). That is a statement of design intent — **the shipped art has not been checked for visual similarity against existing commercial character designs.**
- Origin, coverage, and known unknowns: [`assets/ATTRIBUTION.md`](assets/ATTRIBUTION.md) — **draft, pending owner sign-off.** 849 of the 870 tracked assets were authored by `kaoshock123` (measured); the document covers the Erlang Shen set (169 files) and the audio in detail, while the other 693 have a known author but undocumented generation details.
