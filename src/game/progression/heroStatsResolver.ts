...
import { statsAtStar } from './starAscensionSystem'

/** Base stats at level 1 + placeholder linear growth + star tier multiplier — NON-PRODUCTION. */
export function resolveHeroLevelStats(
  ...
  const starredBase = statsAtStar(character.stats, star)

  return {
    hp: Math.round(starredBase.hp + PLACEHOLDER_LEVEL_STAT_GROWTH.hp * growthSteps),
    atk: Math.round(starredBase.atk + PLACEHOLDER_LEVEL_STAT_GROWTH.atk * growthSteps),
    def: Math.round(starredBase.def + PLACEHOLDER_LEVEL_STAT_GROWTH.def * growthSteps),
    spd: Math.round(starredBase.spd + PLACEHOLDER_LEVEL_STAT_GROWTH.spd * growthSteps),
  }
}
