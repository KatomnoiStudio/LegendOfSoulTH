...
describe('Erlang Shen Skill 2 runtime event', () => {
  it('starts the dedicated three-hound cast and consumes its cooldown', () => {
    ...
    // ชั้นวาดอ่านค่านี้แทนการ hardcode 'skill-1' — ไม่งั้นเฟรมร่าย Skill 2 ไม่มีทางได้เล่น
    expect(runtime.getPlayerAttackAnimationId()).toBe('skill-2')

    runtime.step(1500)
    expect(runtime.getSnapshot().effectEvents.map((event) => event.kind)).toContain('erlang-hound')
  })

  it('reports the running combo step so attack-2 / attack-3 frames can play', () => {
    const state = createRealtimeBattle('trial-01', erlangPlayer())
    if (!state) throw new Error('expected battle state')
    const runtime = new RealtimeBattleRuntime(state)

    runtime.step(1000)
    expect(runtime.getPlayerAttackAnimationId()).toBeNull()

    runtime.requestAttack()
    runtime.step(16)
    expect(runtime.getPlayerAttackAnimationId()).toBe('attack-1')
  })
})
