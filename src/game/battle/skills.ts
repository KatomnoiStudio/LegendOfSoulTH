...
export interface SkillDefinition {
  ...
  name: string
  description: string
  /** ตัวคูณดาเมจสำหรับโจมตีเดี่ยว */
  power?: number
  /** true = โจมตีศัตรูทุกตัวที่ยังมีชีวิต */
...
export function resolveSkill(
  ...
  if (skill.hitsAllEnemies) {
    ...
    for (const enemy of targets) {
      const raw = calcPhysicalDamage(actor.atk, enemy.def, skill.power ?? 1)
      const damage = applyDefense(raw, enemy.defending)
      outcomes.push({
        targetId: enemy.id,
        damage,
        defeated: enemy.hp - damage <= 0,
      })
  ...
  const raw = calcPhysicalDamage(actor.atk, target.def, skill.power ?? 1)
