...
export const ERROR_CODES = {
  ...
  // PvP authority — แยกเป็นรหัสละจังหวะ ไม่ใช่รหัสเดียวคลุมทั้งห้อง
  // เดิมทั้งสี่จุด (สร้าง/เข้าร่วม, ส่ง input, เชื่อมใหม่, ตัดการเชื่อมต่อ) ใช้ PVP_AUTHORITY_FAIL
  // ตัวเดียวกันหมด ซึ่งอ่าน log แล้วแยกไม่ออกเลยว่า "เข้าห้องไม่ได้" ต่างจาก "เข้าได้แต่แพ้เพราะ
  // input ไม่ถึงเซิร์ฟเวอร์" — สองอาการนี้คนละสาเหตุและคนละความรุนแรงโดยสิ้นเชิง
  PVP_ROOM_ACTION_FAIL: 'สร้างหรือเข้าร่วมห้อง PvP ไม่สำเร็จ', // SILENT — UI แสดงข้อความเอง
  PVP_INPUT_FAIL: 'ส่งคำสั่งไปยังเซิร์ฟเวอร์ PvP ไม่สำเร็จ', // SILENT — UI ขึ้น "กำลังรอเชื่อมใหม่"
  PVP_RECONNECT_FAIL: 'เชื่อมต่อห้อง PvP ใหม่ไม่สำเร็จ', // SILENT — ลองใหม่รอบถัดไปเองได้
  PVP_DISCONNECT_FAIL: 'แจ้งออกจากห้อง PvP ไม่สำเร็จ', // SILENT — ผู้เล่นออกจากจอไปแล้ว

  // เต็มจอ/ล็อกแนวหน้าจอตอนเข้าต่อสู้ (src/lib/battle/battleViewport.ts) — SILENT ทั้งหมด
  // เกมเล่นต่อได้ในหน้าต่างปกติ แต่ต้องไม่เงียบสนิท: เบราว์เซอร์ปฏิเสธด้วยเหตุผลต่างกัน
  // (ไม่ได้เกิดจาก user gesture, iOS ไม่รองรับ lock, permission policy) ซึ่งแยกกันไม่ได้เลยถ้าไม่ log
  VIEWPORT_FULLSCREEN_FAIL: 'ขอโหมดเต็มจอไม่สำเร็จ',
  VIEWPORT_FULLSCREEN_EXIT_FAIL: 'ออกจากโหมดเต็มจอไม่สำเร็จ',
  VIEWPORT_ORIENTATION_LOCK_FAIL: 'ล็อกแนวหน้าจอเป็นแนวนอนไม่สำเร็จ',
  VIEWPORT_ORIENTATION_UNLOCK_FAIL: 'ปลดล็อกแนวหน้าจอไม่สำเร็จ',

  // อัญเชิญ (gacha) — เซิร์ฟเวอร์เป็นผู้ตัดสินผลทั้งหมด ฝั่งนี้แค่แสดง
  // SILENT ทั้งคู่เพราะ GachaModal แสดงข้อความให้อยู่แล้ว แต่ต้องแยกสองกรณีในฝั่ง log:
  // ปฏิเสธโดยกติกา (ทรัพยากรไม่พอ/แบนเนอร์ปิด) ต่างจากเรียก RPC ไม่ถึงเลย
  GACHA_PULL_REJECTED: 'เซิร์ฟเวอร์ปฏิเสธการอัญเชิญ',
  GACHA_PULL_FAIL: 'เรียกระบบอัญเชิญไม่สำเร็จ',

  // พัฒนาตัวละคร (อัปสกิล/พรสวรรค์/ปลุกพลัง) — SILENT เพราะทุกจุดขึ้น toast บอกผู้เล่นแล้ว
  // แยกรหัสตามการกระทำเพราะทั้งสามใช้ทรัพยากรคนละอย่างและพังคนละสาเหตุ
  HERO_SKILL_UPGRADE_REJECTED: 'อัปเกรดสกิลไม่สำเร็จ',
  HERO_TALENT_UNLOCK_REJECTED: 'ปลดล็อกพรสวรรค์ไม่สำเร็จ',
  HERO_AWAKENING_REJECTED: 'ปลุกพลังตัวละครไม่สำเร็จ',
  HERO_PROGRESSION_SAVE_FAIL: 'บันทึกผลการพัฒนาตัวละครไม่สำเร็จ',

  // ร้านเติมทอง/หยก — SILENT เพราะ CurrencyShopModal ขึ้น toast ให้อยู่แล้ว
  CURRENCY_TOPUP_FAIL: 'เติมทองหรือหยกไม่สำเร็จ',
