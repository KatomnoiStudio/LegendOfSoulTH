...
vi.mock('@react-three/fiber', () => ({
  ...
  // CharacterModel imports useLoader. It never renders today because SHOW_ARENA_SLOTS is
  // false, but that flag advertises itself as a one-line switch — and vitest throws on first
  // property access, not at import, so flipping it would kill this whole suite with an error
  // pointing nowhere near the change. Stubbed so the failure never misdirects.
  useLoader: vi.fn(),
}))
...
describe('LobbyScene', () => {
  ...
  test('แสดง fallback เมื่อเบราว์เซอร์ไม่รองรับ WebGL2', () => {
    ...
    expect(
      screen.getByText('เบราว์เซอร์นี้ไม่รองรับ WebGL2 — ไม่สามารถแสดงฉาก 3D ได้'),
    ).toBeInTheDocument()
  ...
  test('แสดงรหัสข้อผิดพลาดและรายงานเมื่อ WebGL context หาย แล้วซ่อนเมื่อกู้คืน', () => {
    ...
    expect(
      screen.getByText('การ์ดจอขาดการเชื่อมต่อ — กำลังลองใหม่ ลองรีเฟรชหน้าถ้ายังไม่กลับมา'),
    ).toBeInTheDocument()
