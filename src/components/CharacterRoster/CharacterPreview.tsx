...
/*
  ...
  1.31 คำนวณจริงจาก pixel content ของสไปรต์ทุกตัว/ทุกเฟรม (idle 24 + turnaround 8 ×
  monkey-king/pig-warrior/pilgrim-monk) ด้วย alpha-channel bounding-box scan — เป็นค่าสูงสุด
  ที่ไม่ตัดเนื้อหาจริงแม้แต่พิกเซลเดียวในเฟรมที่ขอบตึงที่สุด (ซุนหงอคง ท่าหมุนเฉียงหลัง
  เหลือ margin เพียง ~24-31% เดิมค่า 1.82/1.55 ตัดอาวุธ/แขนออกจริงในหลายเฟรม ไม่ใช่แค่ pigsy
  อย่างที่ตรวจเจอทีแรก) HetCreep ยืนยันแล้ว (2026-08-08): ใช้ค่าปลอดภัยนี้ไปก่อน ส่วนงาน
  crop sprite source ให้แน่นขึ้น (จะได้ซูมกลับไปใกล้ 1.82 ได้โดยไม่ตัดภาพ) เป็นงานแยกของ
  คนทำ sprite ไม่ใช่งาน CSS นี้

  แก้ค่านี้ที่เดียว ไม่ต้องหาที่สอง — ถ้าจะรีคำนวณหลัง sprite เปลี่ยน ดูสคริปต์วัดที่ใช้ตอน
  ตัดสินใจนี้ใน git history (2026-08-08) เป็นจุดเริ่มต้น
*/
const SPRITE_ZOOM = 1.31
