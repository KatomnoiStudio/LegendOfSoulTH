...
describe('ErrorBoundary', () => {
  ...
  /*
    ห้ามมีปุ่มสำรองข้อมูลบนจอนี้ จนกว่าจะมี server-side export ทำงานได้จริง

    ไม่ใช่การตรึงเรื่องหน้าตา — `accountRepository.supabase.ts` ไม่มีฟังก์ชัน exportSave อีกแล้ว
    (เดิมเป็น stub ที่ hardcode คืน ok:false เสมอ ถูกลบทิ้งพร้อมปุ่มนี้ 2026-08-10 เพราะไม่มีผู้เรียก
    เหลืออยู่เลย) เทสต์นี้ทำให้การเอาปุ่มกลับมาโดยไม่มี export จริงรองรับ เป็นสีแดง
    แทนที่จะเงียบ ๆ ผ่านไปเป็นรอบที่สาม
  */
  test('ไม่มีปุ่มสำรองข้อมูล — ยังไม่มี export ฝั่งเซิร์ฟเวอร์ให้ปุ่มนี้เรียก', () => {
    vi.spyOn(console, 'error').mockImplementation(() => {})
