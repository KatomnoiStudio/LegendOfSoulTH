#!/usr/bin/env node
// Mechanical citation checker for every mandatory-read document (see collectTargets).
// READ-ONLY: reads the documents collectTargets() lists — docs/agent-blueprint/**,
// .agents/rules/**, docs/*-DESIGN-LOCK.md, AGENTS.md, MEMORY.md — and whatever repo
// files they cite; writes nothing back into the repo.
//
...
// Superseded 2026-08-16 by collectTargets(); kept as the historical scope note.
...
function checkCitation(citationText, spans, idx, text, basenameIndex, docState) {
  ...
  if (pathPart && pathPart.includes('/')) {
    ...
    if (docState.lastFullPathByBasename.has(pathPart)) {
      ...
      if (candidates.length === 0) {
        ...
        return {
          ...
          detail: `no file named "${pathPart}" found under repo root`,
        }
    ...
    if (!docState.lastResolvedPath) {
      return {
        verdict: 'MISSING-FILE',
        detail:
          'shorthand `:N` citation with no resolvable preceding file citation in this document',
      }
    }
  ...
  if (minCited < 1 || maxCited > lineCount) {
    return {
      verdict: 'PAST-EOF',
      detail: `cites line ${maxCited} but ${resolvedRel} has ${lineCount} lines`,
    }
  }
  ...
  if (!symbol) {
    return {
      verdict: 'OK',
      detail: 'no adjacent symbol to verify (heuristic has nothing to check)',
      noSymbol: true,
    }
  }
  ...
  if (symRe.test(windowText)) {
    return {
      verdict: 'OK',
      detail: `symbol \`${symbol}\` confirmed within lines ${from + 1}-${to + 1}`,
    }
  }
  return {
    verdict: 'SYMBOL-MISMATCH',
    detail: `symbol \`${symbol}\` not found within lines ${from + 1}-${to + 1} of ${resolvedRel} (heuristic — verify by hand)`,
  }
}

// ---------- 5. driver ----------
/**
 * Every mandatory-read document whose citations this gate covers.
 *
 * Widened 2026-08-16 (audit item B4). It used to read `docs/agent-blueprint` and nothing
 * else, which is measurably why a dangling binding reference in `MEMORY.md` survived being
 * CONFIRMED by an audit on 2026-08-12 and was still live four days later, and why the gacha
 * lock's dead pointer needed a hand sweep. The mechanism the rules estate's own entry gate
 * demands already existed and was ~30 lines short of covering the estate it governs.
 *
 * Returns `{ label, absPath }` pairs — label is what appears in the report.
 */
function collectTargets() {
  ...
  const pushFile = (rel) => {
    ...
    if (fs.existsSync(abs)) targets.push({ label: rel, absPath: abs })
  }
...
function main() {
  ...
  const contractFiles = collectTargets()
  ...
  for (const { label: file, absPath } of contractFiles) {
