 ...
 *
 * รายการนี้เคยมีเจ็ดตัว HetCreep สั่ง backfill 2026-08-19 และห้าตัวปิดไปแล้ว — v0.2.0, v0.8.3,
 * v0.10.0, v0.11.0, v0.11.1 ชี้ที่ commit สุดท้ายที่ package.json อ่านได้เป็นเวอร์ชันนั้นจริง
 * (ยืนยันทีละตัวก่อนสร้าง ไม่ได้เดาจาก subject) เป็น signed annotated ตาม `tag.gpgSign` ของรีโป
 * โดยข้อความของแต่ละ tag ระบุว่าเป็น backfill และ tagger date คือวันที่ backfill ไม่ใช่วันปล่อย
 * เช็คนี้เป็นตัวที่ฟ้องว่ารายการล้าสมัยหลัง push — ทิศทาง "ในรายการแต่มีลิงก์แล้ว" ทำงานจริง
 *
 * **สองตัวที่เหลือ tag ไม่ได้ ไม่ใช่ยังไม่ได้ทำ** — วัด 2026-08-19: 74 commit แตะ package.json
 * มีเวอร์ชันที่เคยมีจริง 38 ตัว และ 0.7.2 กับ 0.7.3 ไม่อยู่ในนั้นเลย CHANGELOG จึงมีหัวข้อของ
 * release ที่ไม่เคยเป็น version ใน package.json การ tag คือการเลือก commit ขึ้นมาเอง = ประดิษฐ์
 * บันทึก ไม่ใช่ backfill ปิดรายการนี้ได้ทางเดียวคือลบสองหัวข้อนั้นออกจาก CHANGELOG ซึ่งเป็น
 * การตัดสินใจของเจ้าของ ไม่ใช่ของ agent
 *
 * ทางกลับกันยังเปิดอยู่และไม่มีเช็คไหนจับ: `v0.7.1` กับ `v0.8.0` เป็น tag ที่ไม่มีหัวข้อใน
 * CHANGELOG เลย — เช็คนี้เดินจากหัวข้อไปหาลิงก์ ไม่ได้เดินจาก tag กลับมาหาหัวข้อ
 */
const KNOWN_UNTAGGED = ['0.7.2', '0.7.3']
