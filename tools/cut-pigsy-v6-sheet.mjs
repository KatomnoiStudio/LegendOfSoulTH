/**
 * ตัดสไปรต์ตือโป๊ยก่ายชุด v6 จาก assets/archive/characters/pigsy-v6-walk-sheet.png
 *
 *   node tools/cut-pigsy-v6-sheet.mjs [--preview]
 *
 * ชีตนี้เป็นตาราง 8 แถว (ทิศ) x 8 คอลัมน์ (เฟรม) บนพื้นดำสนิท มีป้ายชื่อทิศเป็นตัวอักษร
 * อยู่คอลัมน์ซ้ายสุด (จบที่ x=81 ส่วนสไปรต์เริ่ม x=115 ห่างพอจนไม่เสี่ยงถูก closing เชื่อมติดกัน)
 *
 * วาดมาครบ 8 ทิศจริงรวม RIGHT/UP-RIGHT/DOWN-RIGHT จึงไม่ต้องพลิกกระจกสร้างทิศไหนเอง
 * (สเปกที่ส่งมาบอกว่าชีตไม่มีทิศขวาให้พลิกจากซ้ายเอา แต่ตรวจแล้วมีจริงทั้งสามแถว
 * และผ้าคลุมพลิ้วคนละทางกับฝั่งซ้าย ใช้ของจริงจึงถูกต้องกว่าพลิกกระจก)
 *
 * เกมใช้ทิศละ 8 เฟรมพอดีกับชีตนี้ จึงใช้ครบทุกเฟรมโดยไม่ต้องเลือก
 */
...
const SHEET = join(ROOT, 'assets', 'archive', 'characters', 'pigsy-v6-walk-sheet.png')
...
const ROWS = [
  { band: [12, 131], direction: 'down' }, // DOWN (FRONT)
  { band: [137, 251], direction: 'down-left' }, // DOWN-LEFT
  { band: [254, 364], direction: 'left' }, // LEFT
  { band: [366, 474], direction: 'up-left' }, // UP-LEFT
  { band: [476, 579], direction: 'up' }, // UP (BACK)
  { band: [585, 696], direction: 'up-right' }, // UP-RIGHT
  { band: [705, 822], direction: 'right' }, // RIGHT
  { band: [833, 970], direction: 'down-right' }, // DOWN-RIGHT
]
...
async function main() {
  ...
  const render = async ({ cell, feet }, file, mirror = false) => {
    const { bbox } = cell
  ...
  for (const { direction } of ROWS) {
    ...
    for (let frame = 0; frame < FRAME_COUNT; frame++) {
      await render(picked[frame % picked.length], join(WALK_DIR, `pigsy-walk-${direction}-${frame}.png`), mirror)
    }
  }
  console.log(`เขียนเฟรมเดินแล้ว ${ROWS.length * FRAME_COUNT} ไฟล์ (ฝั่งขวา 3 ทิศพลิกจากฝั่งซ้าย)`)
  ...
  if (PREVIEW) {
    ...
    const previewPath = join(ROOT, 'pigsy-v6-preview.png')
    await sharp({
