 ...
 *
 * วัดจริงจาก build วันที่ 2026-08-12, master @ 9c57d8a — ตัวเลขทุกตัวด้านล่างคัดลอกจาก
 ...
 *   แอปสูงสุด     60.9 KB gzip (App-*.js)      → เพดาน  70 KB (เผื่อ 14.9% — แคบ ตัวเลขจริง
 *                                                 ไม่ได้ปัดให้ดูดี เพดานยังไม่ขยับเพราะยัง
 *                                                 ไม่มีการวัดที่บอกว่าควรขยับไปเท่าไหร่)
 *   (เผื่อ = เพดาน/ค่าจริง − 1 ไม่ใช่ส่วนต่างจากเพดาน — เขียนสูตรไว้เพราะเคยอ่านผิดมาแล้ว)
 *   (accountRepository.supabase-*.js 58.0 KB นับเป็น app แล้ว ไม่ใช่ vendor — ดูเหตุผลที่
...
async function main() {
  ...
  try {
    ...
    const why =
      cause?.code === 'ENOENT' ? 'รัน "npm run build" ก่อน' : `อ่านโฟลเดอร์ไม่ได้: ${cause}`
    console.error(`ไม่เจอ ${ASSETS_DIR} — ${why}`)
    process.exitCode = 1
    return
  }
