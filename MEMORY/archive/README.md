...
- **`MEMORY/<system>.md`** (e.g. `MEMORY/25-backend-server-authority-system.md`) — a belt
  caretaker seat's own memory. Scope: **this machine, the belt system**; mandatory for the
  seats listed in `docs/AGENT_REGISTRY.md`, and not part of the shared record.

A belt seat writes its own `MEMORY/<system>.md`. The belt end composes the root index entry.
Everyone, belt or not, still reads root `MEMORY.md` first — that never changed.
