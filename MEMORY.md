...
> **Last Updated (prior)**: 2026-08-11T06:50:00+07:00 by `Claude Code` (audit-#36 caretaker) — audit #36 verified STALE: `loadPlayer`'s 8 queries have all been guarded since `0bee26d`; the real open gap was its test, which hand-listed 5 of the 8 tables. List now derived from the real calls, all 8 covered, red reproduced by re-injecting the old shape (item 195). No source change.
...
> **RULES_VERSION last synced: 35** (2026-08-12, Operator: HetCreep, Agent: Claude Code (belt-end main) — mandate 2 of `agent-memory-law.md`/`AGENTS.md` sharpened to an explicit continuity purpose, and the overdue 2026-08-10 archive sweep applied by hand (item 208); see items 205-208)
> **(prior) RULES_VERSION last synced: 32** (2026-08-12, Operator: HetCreep, Agent: Claude Code (belt-end main) — an ENTRY GATE added to `rules-freshness-check.md`: a new rule must name the incident it prevents, the mechanism that enforces it, and the condition under which it retires, or it does not get written. From a gold-standard audit against Vite/Kubernetes/Godot/Svelte/Mindustry/Veloren/Excalidraw/TanStack: this repo carries ~498 KB of mandatory-read governance against 1.67 MB of source, and every duplication and dead-code finding in that audit happened while 23 rules and 59 rule files were in force. Rule 21 proved the point — four days standing, never obeyed once. The gate binds ADDITIONS only; standing rules are still reviewed by their revalidate stamps. See the audit at docs/AUDIT-2026-08-12-v0.19.0.md)
> **(prior) RULES_VERSION last synced: 31** (2026-08-12, Operator: HetCreep, Agent: Claude Code (belt-end main) — rule 21 (working-directory concurrency lock) RETIRED at the owner's direction, first rule this repo has removed rather than added. The lock file it mandated was never created once, nothing read it, and no agent followed it; the collision is controlled by the operator switching between the Antigravity and Claude sessions only after a turn ends. Number retired in place, NOT renumbered — rules 22/23 and `belt-system-law.md`'s "rules 1–22" still read correctly; see item 205)
...
- **209.** Ring 0 / Ring 1 retired (rule 6, in place) and the belt moved to the gitignored `.claude/belt/` — two constructs that commanded one machine while every machine paid to read them. Every mechanism survived the de-ringing (PR-only intake, sole merger, fresh-tip re-check, owner-locked claims); `ring0-traffic-control-law.md` → `merge-control-law.md`. `docs/BELT-PORT.md` is now the installer, with install-complete and verify-before-deleting checklists. `RULES_VERSION` 33 → 34. — `MEMORY/archive/201-225.md`
