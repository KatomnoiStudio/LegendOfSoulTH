...
180. **Wukong Erlang-style 12-frame movement cycle (2026-08-10, Operator: HetCreep; Agent: Codex/GPT-5)** — normalized the Wukong run reference into exactly 12 transparent 640x512 frames, aligned the bottom anchor and X center across frames, removed disconnected generated artifacts, and wired the in-place cycle into AdventureScene movement and running. Runtime assets are under `assets/raw/characters/monkey-king-run-erlang-style-local/` and `public/characters/monkey-king-run-erlang-style-local/`; the preview remains a local generated artifact. Typecheck, lint, build, and the targeted walk-kit test passed; the full Vitest suite timed out twice without a failure summary and remains an environment/test-runner limitation to investigate.
