...
> **Last Updated**: 2026-08-12T18:10:00+07:00 by `Codex` (`DemoGODRTX`) — **PR #113 rebased and refreshed with Wukong AutoSprite side-view art**: 25 idle, 25 run, and 25 attack frames now load in the lobby, battle, and adventure scene, with mirrored left-facing movement and alpha-measured sprite calibration. The PR branch is rebuilt on current `master`, so the prior merge conflict is removed. (item 216.)
...
> **Last Updated (prior)**: 2026-08-12T16:10:00+07:00 by `Claude Code` (main) — **one day, one record of it.** `docs/OPEN-FINDINGS-2026-08-12.md` is folded into `docs/AUDIT-2026-08-12-v0.19.0.md` as §0b and deleted; the audit already cross-referenced it, so the two were one document pretending to be two. Both coverage ceilings are kept side by side rather than merged into a single softer claim — the earlier scan was pre-filtered by an unsanctioned exclusion list, which the audit was not. **The three HIGH findings are now rows 34-36 in `TASKS.md`** with file:line evidence and a named fix shape, because a finding that lives only in a report is not claimable work. Every inbound reference repointed (`TASKS.md` ×3, `docs/README.md`, `MEMORY.md` ×2, three self-references inside the audit); the two remaining mentions of the old filename are deliberate historical statements. `npm run ci` green (item 214).
...
> **RULES_VERSION last synced: 37** (2026-08-12, Operator: HetCreep, Agent: Claude Code (main) — mandate 2 of `agent-memory-law.md`/`AGENTS.md` sharpened to an explicit continuity purpose, and the overdue 2026-08-10 archive sweep applied by hand (item 208); see items 205-208)
...
- **216.** PR #113 rebuilt on the current `master` with Wukong AutoSprite side-view frames for lobby, battle, and adventure. — `MEMORY/archive/201-225.md`
