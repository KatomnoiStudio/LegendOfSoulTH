...
> **Last Updated (prior)**: 2026-08-11T18:40:00+07:00 by `Claude Code` (main) — **v0.19.0 released** (`08b2c0c`, tag `v0.19.0`): the sprite pipeline gets a compiler. Fourteen commits had landed since 0.18.0 with `[Unreleased]` still empty; the changelog now records them. Standing/walking size spread 8.0–75.1% → 0.02–0.55%, feet at 356.0000 for all ten families, preload 144 requests → 96, and a 22-test frame contract that compares the declared table against alpha-measured art. `L1` published as still open in the character roster rather than quietly carried (item 203, task #112).
> **Last Updated (prior)**: 2026-08-11T12:55:00+07:00 by `Claude Code` — task #93 split at the gate: the honest login error shipped, the password-recovery half dropped until custom SMTP exists (without one it was a 100%-reliable account-existence oracle). Two more enumeration branches deleted from the login path and recorded in `SECURITY.md`; 16/16 mutations killed (item 197).
