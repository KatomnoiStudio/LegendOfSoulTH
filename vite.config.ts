...
export default defineConfig(({ command }) => ({
  ...
  build: {
    /*
      ...
      ── firefox115 ไม่ใช่ 114 และเหตุผลอยู่ตรงนี้ เพื่อไม่ให้ใครยกกลับลง (2026-08-16) ──
      `target` ของ esbuild ลด **ไวยากรณ์** เท่านั้น มัน **ไม่ polyfill เมธอด built-in**
      โค้ดที่ ship อยู่ใช้ `Array.prototype.toSorted`/`toReversed` 11 จุด (PvPAuthorityEngine 6 ·
      stageConfig 2 · WorldChat · chatStorage · combatCameraFraming) ซึ่งต้องการ **Firefox 115**
      (caniuse ยืนยันสด 2026-08-16 — Chrome 110 · Safari 16.0 · Edge 110)
      ที่ floor 114 ผู้ใช้ Firefox 114 จะได้ `TypeError` ไม่ใช่ประสบการณ์ที่ด้อยลง และ
      **build · typecheck · lint · workflow ทั้งหกตัวมองไม่เห็นเลย**

      การยกขึ้นไม่ได้ทิ้งใคร: Firefox 114 ออก 6 มิ.ย. 2023 และ**หมดซัพพอร์ต 4 ก.ค. 2023**
      วันที่ 115 ออก — ได้แพตช์รวมประมาณหนึ่งเดือน ส่วน **115 คือ ESR ตัวสุดท้ายที่รองรับ
      Windows 7/8/8.1 และ macOS 10.12–10.14** (แพตช์ล่าสุด 115.38.0 เมื่อ 21 ก.ค. 2026)
      คนที่ติดอยู่บน OS เก่าจึงอยู่บน 115 ตามนิยาม ไม่มีใครอยู่บน 114 โดยตั้งใจ
      **พื้นเดิมถูกตั้งต่ำกว่าช่วงที่ Mozilla ซัพพอร์ตอยู่แล้วตั้งแต่แรก — นี่คือการแก้ ไม่ใช่การลดการรองรับ**

      เงื่อนไขที่ทำให้ต้องกลับมาดูบรรทัดนี้: ใช้ built-in ตัวใหม่กว่านี้เมื่อไหร่ ให้เช็คเวอร์ชัน
      รองรับก่อน แล้วยก floor อย่างตั้งใจ — ไม่ใช่ค้นพบตอนผู้เล่นเจอ TypeError
    */
